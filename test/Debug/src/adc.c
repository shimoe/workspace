...
volatile unsigned int ADCValue[ADC_NUM];
volatile unsigned int ADCIntDone = 0;

#if BURST_MODE
volatile unsigned int channel_flag; 
#endif
...
void ADCInit( unsigned int ADC_Clk )
{
  ...
  LPC_ADC->CR = ( 0x01 << 0 ) |  /* SEL=1,select channel 0~7 on ADC0 */
	(((SystemFrequency/LPC_SYSCON->SYSAHBCLKDIV)/ADC_Clk-1)<<8) |  /* CLKDIV = Fpclk / 1000000 - 1 */ 
